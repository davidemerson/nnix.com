...
This is the one list I implement as a tag rather than as a place, because dependencies arise in every context and I don't want four copies of the same list. Anything blocked on someone else gets `waiting-for`, wherever it already lives. The `waiting, undated` view exists to catch the failure mode: a dependency with no follow-up date is a dependency you have silently stopped tracking, so that view should always be empty.

## tags
Tags allow me to sort the jumble of things in the various lists into contexts. Contexts for me fall into three classes, and each class gets a prefix so the tag list stays legible and so filters can match a whole class at once:

* `p-` people I deal with a lot, whether personal or work related. `p-personone`, `p-persontwo`.
* `w-` my various jobs, employers, clients, and the recurring meetings attached to them. `w-myjob`, `w-exec-weekly`.
* `c-` categories. `c-reading`, `c-gift`, `c-phone`, `c-gtd`.

The prefixes are the whole point. Without them the tag list is a flat pile of proper nouns and common nouns with no way to tell which is which, and every filter has to enumerate tags by hand. Enumerated filters rot the moment someone leaves or a category gets renamed. With prefixes, `@p-*` is every task tied to a human being, and that view keeps working without maintenance.

Structural tags, which describe a task's state rather than its context, stay unprefixed. There is currently exactly one: `waiting-for`.

I use these tags to build filters and views as appropriate. If I need to see what phone calls I need to make or schedule, I call up `c-phone`. If I need to see everything I owe a particular person, I call up their `p-` tag. If I need to see everything I'm doing for a given client, I call up their `w-` tag.

A caution learned the hard way: renaming a personal tag does not reliably propagate to tasks living in shared projects. After any rename, check the shared label list too, or you will end up with two tags that look identical and filter differently.
...
## implementation
Everything above is method. This is how it's actually built, in Todoist, as of this writing. I keep this section current because a method you can't locate in the tool is a method you don't have.

### structure
One project per context. Inside each, the lists are sections, not separate projects:

* `Personal`, `dayjob`, `consulting`, `Board Service` each contain `next`, `recurring`, and `ideas`.
* `FDCP` is a property and keeps sections per trade (procurement, exterior, interior, and so on) plus a `recurring` section for the maintenance backbone.
* Genuinely shared projects stay separate and untouched, because collaboration is one of the two things that earns project status.

Nothing sits unsectioned. An unsectioned task is a task in no list, which is the same as a task in no system.

The rules that keep this from sprawling again, checked quarterly:

* Not more than ten top-level projects.
* Not more than ten sections per project.
* A project must be shared with someone, or must hold information that cannot commingle with anything else. Otherwise it is a tag. This is the same test as before, but written down now, because an untested heuristic drifts.

### views
The lists are where things live; the views are how I see across them. Each maps to a step in a review.

* `Today` — `today | overdue`, everything due across every context.
* `Next actions` — every `next` section, plus FDCP's trade sections.
* `Next: undated` — the same, filtered to items with no date. Most next actions should have a date; this is the backlog that quietly stopped being actionable.
* `Waiting for` and `Waiting, undated` — the dependencies list and its failure mode.
* `Ideas` — every `ideas` section.
* `Recurring` — every `recurring` section. Mostly used quarterly to catch rot, since recurring tasks accumulate references to tools you stopped using years ago.
* `People` — `@p-*`, everything tied to a human.
* `Weekly review` — a single multi-query view rendering inbox, waiting-for, undated next actions, and ideas in one screen. Four steps of the weekly review in one click.

A note on filter syntax, since it took some checking: section names match across all projects, so `/next` returns every next-actions section at once, and tag prefixes support wildcards, so `@p-*` works. Those two facts are what make one flat set of views cover an arbitrary number of contexts.
...
* Review all Next Actions (`Next actions`) to ensure things are completed which were completed, and to reschedule, reshuffle, or elaborate on other tasks in there before next week starts. Then check `Next: undated` and either give those items a date or admit they belong in Ideas.
* Review Delegations (`Waiting for`) to be sure nothing is falling through the cracks, particularly to be sure everything has a date on which you will follow up about that thing. `Waiting, undated` should be empty.
...
* Rethink any Projects which should be demoted to tags, or tags which should be promoted to projects. Apply the test rather than the vibe: shared, or cannot commingle, or it's a tag. Check the counts against the caps while you're in there.
* Work the `Recurring` view end to end. Recurring tasks rot in a way one-time tasks don't, because nothing forces you to reread them. Look for references to tools you no longer use, recurrence strings that have accreted junk over the years, and tasks whose whole premise has expired.
* Clean up old stuff to storage where it's no longer needed.
* Make sure any integrations between systems (such as calendar integrations with your task management app, availability integrations with the links you send out for scheduling, or any agent connectors with access to your mail and tasks) are working and properly configured. For anything holding an OAuth grant, confirm the scope is still the minimum you need and revoke what isn't.
...
* not more than ten subcategories per second or third level taxonomy
