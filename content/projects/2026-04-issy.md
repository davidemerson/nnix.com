...
issy is a text editor that looks like a printed page, not a terminal application. It's written in Zig with zero external dependencies, runs on Linux, macOS, and OpenBSD, and ships as a single binary. No runtime, no package tree. Current release is v1.4.1.
...
The print theme is tuned for ink on white paper and never inherits the TUI theme. Recommended fonts: Berkeley Mono, Iosevka, JetBrains Mono, Commit Mono. Font metrics are sanity-checked on load, so a malformed or hostile font can neither panic nor stall the export.

### multiple cursors

`Ctrl+D` selects the word under the cursor and adds a cursor at the next occurrence. Press it again to keep adding. Each press arms a rename: the word is highlighted at every secondary cursor, and the first character you type replaces it everywhere in one shot. From there, every edit, whether typing, Enter, Tab, backspace, delete, or paste, applies at all cursors simultaneously, and `Ctrl+Z` undoes the whole multi-cursor tick as one step. Any non-typing key (an arrow, a paste) disarms the pending rename, so a later insert can never eat bytes at stale positions.
...
Copy and cut push to the OS clipboard via OSC 52, so yanking out of an SSH session lands in the host clipboard without extra plumbing (tmux wants `set -g set-clipboard on` first). Copies over 100KB stay internal-only with a status notice, because pasting a silently truncated clipboard somewhere else would be data corruption.

Since v1.3.0 the read direction works too. `Ctrl+V` asks the terminal for the system clipboard over OSC 52, so text copied in any other application pastes straight in, and middle-click pastes the primary selection the same way. The read is synchronous: issy emits the query, waits briefly for the reply, then inserts. If the terminal has OSC 52 read disabled (common over SSH, and some terminals allow the write but not the read), it falls back to issy's own internal clipboard, so nothing regresses. `system_clipboard = false` in `~/.issyrc` keeps `Ctrl+V` internal-only. The terminal's native paste keys, `Ctrl+Shift+V` and `Shift+Insert`, always work as well, arriving as a bracketed paste (DECSET 2004).

However it arrives, a paste is captured as one verbatim blob and inserted as a single undo step. Auto-indent is suppressed, tabs land as literal `\t`, `\r\n` normalizes to `\n` (the buffer is LF-only in memory, and CRLF files get their endings back on save), and a newline inside the paste never confirms a prompt or submits a search. That last one fixed a real data-loss bug: pasting a multi-line snippet while the "discard unsaved changes?" prompt was up used to answer it.

### wide glyphs

CJK and emoji occupy two terminal cells; combining marks occupy none. Since v1.4.0 the renderer, cursor math, mouse mapping, soft-wrap, and horizontal scroll all measure display width per codepoint instead of assuming one cell per character, so wide text renders in place rather than as blanks, the cursor lands on the glyph you'd expect, and a click on either half of a wide glyph snaps to it. The bytes on disk were always right; only the screen was wrong.

{{ img(id="/images/issy/wide-glyphs.png", alt="cjk and emoji in a markdown table with aligned columns") }}

### path completion

`Ctrl+O` opens the file prompt seeded with the current directory. Type a partial directory or filename and press `Tab` to auto-complete against what's on disk. The completion ghost shows the remainder with a `[Tab]` marker so it's clear what a press will fill in. The same prompt backs Save-As for a buffer with no filename.

{{ img(id="/images/issy/path-completion.gif", alt="path completion in the open-file prompt") }}

### swap files

While a buffer has unsaved changes, issy writes them to a sibling `.<name>.swp` file every couple of seconds, and removes it on save, reload, file switch, or clean exit (SIGTERM and SIGHUP shut down gracefully and clean up too). If issy (or your SSH session, or your laptop battery) dies with edits in flight, the next open of that file points you at the swap in the status bar. It never auto-loads the swap; it just tells you it's there. A clean quit only removes swaps this session wrote, so it can never destroy a crash-recovery swap left by a previous one. On by default, `swap_files = false` in `~/.issyrc` turns it off.
...
Some behaviors are on by default because I always want them. Long lines soft-wrap at the right margin; the buffer itself is never modified. Opening a file scans the first 100 lines and, if more than 60% agree on tabs or spaces, overrides the configured indent style for that file. Trailing whitespace and indentation that doesn't match the detected file style get a faint highlight, quiet enough to ignore until you care. Matching `()[]{}` pairs highlight as the cursor moves. Opening a file reads to real end-of-file rather than trusting `stat`, so files under `/proc` and `/sys` that report a size of zero open with their content.

Save is atomic and honest about failure. issy writes a uniquely named `O_EXCL` temp sibling, fsyncs it, renames it into place (following symlinks to rewrite the target and preserving permission bits), and fsyncs the parent directory. An fsync failure propagates instead of being swallowed, so a truncated temp can never be renamed over the good file while the status bar says `Saved.`, and errors carry their reason (`Save failed: AccessDenied`). `Ctrl+S` works from every mode: inside search, replace, or the help overlay it saves and returns to normal, and at the unsaved-changes prompt it saves and stays.
...
Drops `issy` at `~/.local/bin/issy`, verifies an Ed25519 signature over the release manifest against a public key baked into the script, verifies the binary's SHA-256 against that signed manifest, seeds `~/.issyrc` with commented defaults if you don't already have one, records the release it just installed in `~/.cache/issy` so the first launch doesn't advertise an update you already have, and wires up the opt-in auto-update path. Prebuilt binaries cover Linux amd64/arm64 and OpenBSD amd64. On platforms without a prebuilt (or when you invoke the installer on macOS), it falls through to `zig build -Doptimize=ReleaseSafe` from source, with either Zig 0.15.x or 0.16.x on `PATH`.
...
Release builds check for newer versions on startup. The check is a one-shot HTTPS request to a `commit.txt` asset on the latest GitHub release (the release commit and its timestamp), made by a detached grandchild process so the editor itself never blocks on the network. If that release is newer than the running binary, compared by commit timestamp rather than by a merely different SHA, the footer shows `update available: <sha>`. The distinction matters: a stale cache left over from a manual `brew upgrade` used to advertise the very version you'd just replaced. Dev builds skip the check entirely; only builds stamped as releases (a build from a clean tree, CI, or the Homebrew stable formula) participate.
...
1. The background worker downloads `sha256sums.txt` and its Ed25519 signature, verifies the signature against the public key embedded in `src/update_key.zig`, and checks the manifest's embedded commit binding and monotonic epoch. A replayed older release, authentic but stale, is rejected as a downgrade. Then it downloads the platform-specific binary and checks it against the signed manifest. It declines to stage anything that isn't strictly newer, and when the binary isn't writable by you (a root-owned or distro-packaged install) it downloads nothing at all: the check stays notify-only, with no wasted bandwidth and no recurring failure notice.
2. The verified binary is staged under `~/.cache/issy/issy.staged` and the footer switches to `update staged: <sha>`.
3. Next time the buffer is clean and the editor has been idle for 60 seconds, it re-verifies the staged binary against the cached signed manifest, including a check that the release it names is strictly newer than the running build (an authentic but older staged binary is refused rather than installed as a downgrade). It then writes a small resume record, snapshots the current binary to `~/.cache/issy/issy.prev`, atomically renames the staged binary over its own executable (falling back to a copy-and-rename inside the target directory when the cache lives on a different filesystem), tears down the terminal, and `execve()`s the new binary with `--resume <path>` plus whatever `--config`, `--theme`, `--font`, or `--no-config` flags you launched with. The terminal state survives `execve`, so the visible effect is a single re-render and the cursor lands back where it was.
4. If anything goes wrong (non-writable binary, signature mismatch, dirty buffer, failed rename) the editor falls back to notify-only for the rest of the session and keeps running the old version.
...
To roll back after an apply: `issy --rollback` swaps the previous binary back in a one-shot atomic rename, after verifying the snapshot against the checksum recorded at apply time. That check fails closed: a checksum sidecar that exists but doesn't parse or match refuses the rollback rather than proceeding unchecked.

The whole chain is covered end to end. `build.zig` can generate test-only seams (a base URL, a public key, an idle threshold) that let an integration suite drive a real update against a loopback fixture server signed with a throwaway key. A plain `zig build` rewrites those seams to null on every configure, so they fold away at comptime and cannot reach a release binary; an override build stamps `test-update` into `--version`, which the suite asserts is absent from the shipped binary.

If you'd rather not think about any of this, `notify_updates = false` disables the check entirely.

### hardening

Between v1.1.0 and v1.4.1, issy went through repeated rounds of adversarial audits: independent review agents combing the code for crashes, data loss, and hostile-input problems, with every confirmed finding fixed and pinned by a regression test. Some favorites from the pile:
...
- An fsync failure during save was swallowed, so a truncated temp file could be renamed over the good copy while the status bar reported `Saved.`.
- Backspacing a non-ASCII character with multiple cursors deleted exactly one byte per cursor, leaving dangling UTF-8 lead bytes that `Ctrl+S` then wrote to disk.
- A newline inside a pasted snippet answered the discard-unsaved-changes prompt. Paste is now inert text in every mode.
- The undo cap could evict entries from an operation still in progress (a replace-all with more matches than the cap), leaving it silently half-undoable.
- An authentic manifest for an older release passed the signature and hash checks perfectly, so after a manual upgrade the next idle minute could reinstall the release you'd just upgraded away from.
- A crafted font with `units_per_em = 0` drove every metric through a divide by zero and hard-panicked PDF export; another declaring `ascender = 32767` and `descender = -32768` overflowed unwidened `i16` arithmetic and panicked in ReleaseSafe, the mode users actually run. Out-of-spec values now clamp, and the math is widened.
...
- Only SIGTERM and SIGHUP had handlers, so a real SIGSEGV left the terminal in raw mode on the alternate screen with the crash message invisible. Fatal signals now restore the terminal and re-raise, preserving the exit status and core dump.

The same passes bought a panic handler that restores the terminal on every exit path, escape-sequence parsing that survives sequences split across reads (SSH), the same bounded retry for a multi-byte character split across reads (so a typed `é` on a laggy link can't decode as two replacement characters), and CRLF files that normalize to LF in memory and round-trip byte-for-byte on save.
...
bash tests/run_tests.sh     # 25 integration suites via expect
```

There are 304 distinct unit tests; the runner reports well over a thousand because each source file compiles as its own test unit and re-runs the modules it imports. The integration suite launches the real binary in a PTY under a scratch `HOME` so it can't touch real state, sends keystrokes, and verifies outcomes by checking saved file contents; a hang is converted into a kill and a `FAIL` rather than a stall. It's slower than pure unit tests but it catches the kind of bug where the editor thinks it saved a file but actually didn't. The OpenBSD CI job runs the same suites inside a real OpenBSD 7.9 amd64 VM on every push, and the Linux job runs them on both Zig 0.15.2 and 0.16.0, so neither OpenBSD nor the newer toolchain is a "should work" target. They're gates.
